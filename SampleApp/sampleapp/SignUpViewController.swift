...
class SignUpViewController: SuperViewController, UITextFieldDelegate {
    ...
    var userName, userEmail, userPassword : UITextField!
    ...
    func setupContent()
    {
        ...
        userName = TextField(frame: CGRectMake(Dimensions.MARGIN_LEFT, CGRectGetMaxY(label.frame)+Dimensions.MARGIN_LEFT/2.0, contentWidth, 44.0))
        ...
        userEmail = TextField(frame: CGRectMake(Dimensions.MARGIN_LEFT, CGRectGetMaxY(userName.frame)+Dimensions.MARGIN_LEFT/2.0, contentWidth, 44.0))
        ...
        register.addTarget(self, action: "registerPress:", forControlEvents: .TouchUpInside)
        register.setTitle("Register new user", forState: .Normal)
    ...
    func registerPress(sender : UIButton)
    {
        ...
        u.save {
            ...
            if result.succeeded()
            {
                ...
                if !newUser.isActivated.boolValue
                {
                    newUser.activate {
                        (result : SResponse!) -> Void in
                        if result.succeeded()
                        {
                            self.alert.message = "User " + u.name + " (" + u.email  + ") created and activated."
    ...
    {
        /**
        Before uses of LGSocialAuthenticator with Facebook login:
    ...
    {
        /**
        Before uses of LGSocialAuthenticator with Twitter OAuth:
        ...
        **/
        ...
        askEmail {
            ...
            if email != nil
            {
                self.twitterSignUp(email!)
            }
            else
            {
                self.alert.message = "Invalid input."
                self.presentViewController(self.alert, animated: true, completion: nil)
            }
        }
    }
    ...
    {
        /**
        Before uses of LGSocialAuthenticator with Google+ OAuth:
                ...
                user.save {
                    ...
                    if result.succeeded()
                    {
                        ...
                        self.registerPlatform(newUser, completion: {
                            ...
                            if result.succeeded()
                            {
                                self.alert.message = "Signed up" + (newUser.name != nil ? " as " + newUser.name : "")
                                self.presentViewController(self.alert, animated: true, completion: nil)
                            }
                            else
                            {
    ...
    func twitterSignUp(email : String)
    {
        HUD.showUIBlockingIndicator()
